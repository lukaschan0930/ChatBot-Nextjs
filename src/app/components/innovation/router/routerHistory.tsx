...
const RouterHistory = () => {
    ...
    const [routerChatHistory, setRouterChatHistory] = useAtom(routerChatHistoryAtom);
    ...
    const deleteSession = async (id: string) => {
        await setRouterChatHistory(routerChatHistory.filter((session) => session.id !== id));
        if (sessionId === id) {
            setSessionId(routerChatHistory[0].id);
        }
        await fetch("/api/innovation/router/history", {
            method: "DELETE",
    ...
    const updateSessionTitle = async (id: string, newTitle: string) => {
        if (newTitle != "" && newTitle != extractTitleFromMd(routerChatHistory.find(session => session.id === id)?.title || "")) {
            try {
                await fetch("/api/innovation/router/history", {
                    method: "PUT",
                    body: JSON.stringify({ id, title: newTitle }),
                });
                setRouterChatHistory(routerChatHistory.map(session =>
    ...
    useEffect(() => {
        const fetchChats = async () => {
            if (sessionId) {
                setIsLoading(true);
                try {
                    const chats = await fetch(`/api/innovation/router/log?sessionId=${sessionId}`);
