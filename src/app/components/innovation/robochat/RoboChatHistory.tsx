...
import { IRoboChatHistory as ChatHistoryType, IFileWithUrl } from "@/app/lib/interface";
import { roboChatHistoryAtom, roboChatLogAtom, sessionIdAtom, isStartChatAtom, isSidebarVisibleAtom, fileAtom, roboActiveChatAtom } from "@/app/lib/store";
...
const ChatHistory = () => {
    ...
    const [roboChatHistory, setRoboChatHistory] = useAtom(roboChatHistoryAtom);
    ...
    const deleteSession = async (id: string) => {
        await setRoboChatHistory(roboChatHistory.filter((session) => session.id !== id));
        if (sessionId === id) {
            setSessionId(roboChatHistory[0].id);
        }
        await fetch("/api/innovation/robochat/history", {
            method: "DELETE",
    ...
    const updateSessionTitle = async (id: string, newTitle: string) => {
        if (newTitle != "" && newTitle != extractTitleFromMd(roboChatHistory.find(session => session.id === id)?.title || "")) {
            try {
                await fetch("/api/innovation/robochat/history", {
                    method: "PUT",
                    body: JSON.stringify({ id, title: newTitle }),
                });
                setRoboChatHistory(roboChatHistory.map(session =>
    ...
    useEffect(() => {
        const fetchChats = async () => {
            if (sessionId) {
                setIsLoading(true);
                try {
                    const chats = await fetch(`/api/innovation/robochat/log?sessionId=${sessionId}`);
                    const data = await chats.json();
                    if (data.success && data.data && data.data.length > 0) {
                        setRoboChatLog(data.data);
                        setRoboActiveChat(data.data[data.data.length - 1]);
                        setIsStartChat(true);
                    } else {
    ...
    const categorizeSessions = (sessions: ChatHistoryType[]) => {
        ...
        sessions.forEach(session => {
            const sessionTime = moment(Number(session.chats[session.chats.length - 1].createdAt));
                                                    ...
                                                    <div className="w-[200px] flex flex-col gap-1">
                                                        {editingSessionId === session.id ? (
                                                            ...
                                                            <>
                                                                ...
                                                                <div className="text-[12px] text-[#3E3E40]">{moment(session.chats[session.chats.length - 1].timestamp).format("Do MMM YY HH:mm:ss")}</div>
                                                            </>
                                                        )}
                                                    </div>
                                                    {
